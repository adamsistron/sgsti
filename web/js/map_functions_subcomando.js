...
function createMarkerClickHandlerSubComando(marker, text, link) {
  return function() {
    marker.openInfoWindowHtml(
      '<b>' + text.nb_sudcomando + '</b>' +
      '<p>Responsable: '+text.nb_responsable+'</p>'+
      '<p>Telefono: '+text.nu_telefono+'</p>'
...
function createMarkerSubComando(pointData) {
  var latlng = new GLatLng(pointData.coord_y, pointData.coord_x);

  var icon = new GIcon();
  icon.image = "images/comando.png";
  icon.iconSize = new GSize(40, 32);
  icon.iconAnchor = new GPoint(16, 16);
  icon.infoWindowAnchor = new GPoint(25, 7);
  ...
  opts = {
    "icon": icon,
    "clickable": true,
    "labelText": pointData.nb_sudcomando,
    "labelOffset": new GSize(-19, -50)
  };
 
  var marker = new LabeledMarker(latlng, opts);
  var handler = createMarkerClickHandlerSubComando(marker, pointData, pointData.wp);
...
function init() {
  ...
  //manager.refresh();
  // This is a sorting trick, don't worry too much about it.
  subcomando.sort(function(a, b) { return (a.abbr > b.abbr) ? +1 : -1; }); 
	
  batch = [];
 
  for(id in subcomando) {
    batch.push(createMarkerSubComando(subcomando[id]));
  }
