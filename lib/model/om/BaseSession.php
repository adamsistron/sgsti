...
abstract class BaseSession extends BaseObject  implements Persistent {
  ...
  const PEER = 'SessionPeer';
	...
	/**
	 * Get the [optionally formatted] temporal [sess_time] column value.
	 * 
	 *
	 ...
	 */
	public function getSessTime($format = 'H:i:s')
	{
		if ($this->sess_time === null) {
			return null;
		}
		...
		try {
			$dt = new DateTime($this->sess_time);
		} catch (Exception $x) {
			throw new PropelException("Internally stored date/time/timestamp value could not be converted to DateTime: " . var_export($this->sess_time, true), $x);
		}
	...
	public function setSessId($v)
	{
		...
		if ($this->sess_id !== $v) {
			$this->sess_id = $v;
			$this->modifiedColumns[] = SessionPeer::SESS_ID;
		}
	...
	public function setSessData($v)
	{
		...
		if ($this->sess_data !== $v) {
			$this->sess_data = $v;
			$this->modifiedColumns[] = SessionPeer::SESS_DATA;
		}
	...
	/**
	 * Sets the value of [sess_time] column to a normalized version of the date/time value specified.
	 * 
	...
	public function setSessTime($v)
	{
		...
		if ( $this->sess_time !== null || $dt !== null ) {
			// (nested ifs are a little easier to read in this case)

			$currNorm = ($this->sess_time !== null && $tmpDt = new DateTime($this->sess_time)) ? $tmpDt->format('H:i:s') : null;
			$newNorm = ($dt !== null) ? $dt->format('H:i:s') : null;

			if ( ($currNorm !== $newNorm) // normalized values don't match 
					)
			{
				$this->sess_time = ($dt ? $dt->format('H:i:s') : null);
				$this->modifiedColumns[] = SessionPeer::SESS_TIME;
			}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->sess_id = ($row[$startcol + 0] !== null) ? (string) $row[$startcol + 0] : null;
			$this->sess_data = ($row[$startcol + 1] !== null) ? (string) $row[$startcol + 1] : null;
			$this->sess_time = ($row[$startcol + 2] !== null) ? (string) $row[$startcol + 2] : null;
			$this->resetModified();
			...
			return $startcol + 3; // 3 = SessionPeer::NUM_COLUMNS - SessionPeer::NUM_LAZY_LOAD_COLUMNS).
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					$this->setSessId($pk);  //[IMV] update autoincrement primary key
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(SessionPeer::SESS_ID)) $criteria->add(SessionPeer::SESS_ID, $this->sess_id);
		if ($this->isColumnModified(SessionPeer::SESS_DATA)) $criteria->add(SessionPeer::SESS_DATA, $this->sess_data);
		if ($this->isColumnModified(SessionPeer::SESS_TIME)) $criteria->add(SessionPeer::SESS_TIME, $this->sess_time);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(SessionPeer::SESS_ID, $this->sess_id);
