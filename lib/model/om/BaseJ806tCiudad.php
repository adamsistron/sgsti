...
/**
 * Base class that represents a row from the 'j806t_ciudad' table.
 *
 * 
 *
 ...
 */
abstract class BaseJ806tCiudad extends BaseObject  implements Persistent {


  const PEER = 'J806tCiudadPeer';
	...
	/**
	 * Initializes internal state of BaseJ806tCiudad object.
	 * @see        applyDefaults()
	 */
	 ...
	 * @return     J806tCiudad The current object (for fluent API support)
	 */
	public function setCoCiudad($v)
	{
		...
		if ($this->co_ciudad !== $v) {
			$this->co_ciudad = $v;
			$this->modifiedColumns[] = J806tCiudadPeer::CO_CIUDAD;
		}
	...
	public function setTxDescripcion($v)
	{
		...
		if ($this->tx_descripcion !== $v) {
			...
			$this->modifiedColumns[] = J806tCiudadPeer::TX_DESCRIPCION;
		}
	...
	public function setCreatedAt($v)
	{
		...
		if ( $this->created_at !== null || $dt !== null ) {
					...
					)
			{
				...
				$this->modifiedColumns[] = J806tCiudadPeer::CREATED_AT;
			}
	...
	public function setUpdatedAt($v)
	{
		...
		if ( $this->updated_at !== null || $dt !== null ) {
					...
					)
			{
				...
				$this->modifiedColumns[] = J806tCiudadPeer::UPDATED_AT;
			}
		} // if either are not null
	...
	public function setCoTransaccion($v)
	{
		...
		if ($this->co_transaccion !== $v) {
			...
			$this->modifiedColumns[] = J806tCiudadPeer::CO_TRANSACCION;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->co_ciudad = ($row[$startcol + 0] !== null) ? (string) $row[$startcol + 0] : null;
			...
			return $startcol + 5; // 5 = J806tCiudadPeer::NUM_COLUMNS - J806tCiudadPeer::NUM_LAZY_LOAD_COLUMNS).

		} catch (Exception $e) {
			throw new PropelException("Error populating J806tCiudad object", $e);
		}
	}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = J806tCiudadPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
    ...
    foreach (sfMixer::getCallables('BaseJ806tCiudad:delete:pre') as $callable)
    {
      ...
      if ($ret)
      {
		...
		try {
			J806tCiudadPeer::doDelete($this, $con);
    ...
    foreach (sfMixer::getCallables('BaseJ806tCiudad:delete:post') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BaseJ806tCiudad:save:pre') as $callable)
    {
    ...
    if ($this->isNew() && !$this->isColumnModified(J806tCiudadPeer::CREATED_AT))
    {
      $this->setCreatedAt(time());
    }

    if ($this->isModified() && !$this->isColumnModified(J806tCiudadPeer::UPDATED_AT))
    {
      $this->setUpdatedAt(time());
		...
		try {
			...
			$con->commit();
    foreach (sfMixer::getCallables('BaseJ806tCiudad:save:post') as $callable)
    {
      call_user_func($callable, $this, $con, $affectedRows);
    }

			J806tCiudadPeer::addInstanceToPool($this);
			return $affectedRows;
		} catch (PropelException $e) {
			$con->rollBack();
			throw $e;
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = J806tCiudadPeer::doInsert($this, $con);
					$affectedRows += 1; // we are assuming that there is only 1 row per doInsert() which
										 // should always be true here (even though technically
										 // BasePeer::doInsert() can insert multiple rows).

					$this->setCoCiudad($pk);  //[IMV] update autoincrement primary key

					$this->setNew(false);
				} else {
					$affectedRows += J806tCiudadPeer::doUpdate($this, $con);
				}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = J806tCiudadPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = J806tCiudadPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = J806tCiudadPeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getCoCiudad(),
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[0], $arr)) $this->setCoCiudad($arr[$keys[0]]);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(J806tCiudadPeer::CO_CIUDAD)) $criteria->add(J806tCiudadPeer::CO_CIUDAD, $this->co_ciudad);
		if ($this->isColumnModified(J806tCiudadPeer::TX_DESCRIPCION)) $criteria->add(J806tCiudadPeer::TX_DESCRIPCION, $this->tx_descripcion);
		if ($this->isColumnModified(J806tCiudadPeer::CREATED_AT)) $criteria->add(J806tCiudadPeer::CREATED_AT, $this->created_at);
		if ($this->isColumnModified(J806tCiudadPeer::UPDATED_AT)) $criteria->add(J806tCiudadPeer::UPDATED_AT, $this->updated_at);
		if ($this->isColumnModified(J806tCiudadPeer::CO_TRANSACCION)) $criteria->add(J806tCiudadPeer::CO_TRANSACCION, $this->co_transaccion);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(J806tCiudadPeer::CO_CIUDAD, $this->co_ciudad);
	...
	/**
	 * Generic method to set the primary key (co_ciudad column).
	 *
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of J806tCiudad (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     J806tCiudad Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @return     J806tCiudadPeer
	 */
	public function getPeer()
	{
		if (self::$peer === null) {
			self::$peer = new J806tCiudadPeer();
		}
		return self::$peer;
	}
	 ...
	 * Otherwise if this J806tCiudad has previously been saved, it will retrieve
	 * related C003tActaAccs from storage. If this J806tCiudad is new, it will return
		...
		if ($this->collC003tActaAccs === null) {
			if ($this->isNew()) {
			   $this->collC003tActaAccs = array();
				...
				$criteria->add(C003tActaAccPeer::CO_CIUDAD_ACTA, $this->co_ciudad);
	 ...
	 * Otherwise if this J806tCiudad is new, it will return
	 * an empty collection; or if this J806tCiudad has previously
	 * been saved, it will retrieve related C003tActaAccs from storage.
	 *
	 * This method is protected by default in order to keep the public
	 * api reasonable.  You can provide public methods for those you
	 * actually need in J806tCiudad.
	 */
	 ...
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this J806tCiudad has previously been saved, it will retrieve
	 * related C004tActaAies from storage. If this J806tCiudad is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
		...
		if ($this->collC004tActaAies === null) {
			if ($this->isNew()) {
			   $this->collC004tActaAies = array();
				...
				$criteria->add(C004tActaAiePeer::CO_CIUDAD_ACTA, $this->co_ciudad);
	 ...
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this J806tCiudad has previously been saved, it will retrieve
	 * related J006tLugarSeguros from storage. If this J806tCiudad is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
		...
		if ($this->collJ006tLugarSeguros === null) {
			if ($this->isNew()) {
			   $this->collJ006tLugarSeguros = array();
				...
				$criteria->add(J006tLugarSeguroPeer::CO_CIUDAD, $this->co_ciudad);
  ...
  {
    if (!$callable = sfMixer::getCallable('BaseJ806tCiudad:'.$method))
    {
      throw new sfException(sprintf('Call to undefined method BaseJ806tCiudad::%s', $method));
    }
